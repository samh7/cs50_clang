...
int main(int argc, char const *argv[])
{
    ...
    printf("Change owed: ");
    scanf("%f", &change1);
    int quarters = getQuarters(change1);
    change1 -= (quarters * 0.25);
    int dimes = getDimes(change1);
    change1 -= (dimes * 0.25);
    int nickels = getNickels(change1);
    change1 -= (nickels * 0.25);
    int pennies = getPennies(change1);
    change1-= pennies;
